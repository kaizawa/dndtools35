...
public class MonsterAbilityRecordController implements Serializable {
    ...
    public MonsterAbilityRecord getSelected() {
        if (current == null) {
            ...
            current.setMonsterAbilityRecordPK(new entity.MonsterAbilityRecordPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle").getString("MonsterAbilityRecordCreated"));
    ...
    public String update() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle").getString("MonsterAbilityRecordUpdated"));
            return "View";
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/Bundle").getString("PersistenceErrorOccured"));
            return null;
    ...
    private void performDestroy() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle").getString("MonsterAbilityRecordDeleted"));
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/Bundle").getString("PersistenceErrorOccured"));
        }
    }
    ...
    public static class MonsterAbilityRecordControllerConverter implements Converter {
        ...
        entity.MonsterAbilityRecordPK getKey(String value) {
            entity.MonsterAbilityRecordPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new entity.MonsterAbilityRecordPK();
            key.setMonster(Integer.parseInt(values[0]));
            key.setAbilityId(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(entity.MonsterAbilityRecordPK value) {
